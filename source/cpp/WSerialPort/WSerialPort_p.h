...
#ifndef WSERIALPORT_BUFFERSIZE
    #define WSERIALPORT_BUFFERSIZE 32*1024
#endif
...
class WSerialPortErrorInfo
{
public:
    explicit WSerialPortErrorInfo(WSerialPort::SerialPortError newErrorCode = WSerialPort::UnknownError,
                                  const QString &newErrorString = QString());
    WSerialPort::SerialPortError errorCode = WSerialPort::UnknownError;
    QString errorString;
};

class WSerialPortPrivate : public QIODevicePrivate
{
    Q_DECLARE_PUBLIC(WSerialPort)
public:
                            WSerialPortPrivate();
    ...
    bool                    flush();
//    bool                    clear(WSerialPort::Directions directions);

    bool                    waitForReadyRead(int msec);
    bool                    waitForBytesWritten(int msec);

    WSerialPortErrorInfo    getSystemError(int systemErrorCode = -1) const;
    void                    setError(const WSerialPortErrorInfo &errorInfo);
    ...
    WSerialPort::SerialPortError error = WSerialPort::NoError;
...
//    OVERLAPPED communicationOverlapped;
    ...
    bool setCustomBaudRate(qint32 baudRate, WSerialPort::Directions directions);
    bool setStandardBaudRate(qint32 baudRate, WSerialPort::Directions directions);
