...
{
public:
    explicit ReadNotifier(WSerialPortPrivate *d, QObject *parent)
...
private:
    WSerialPortPrivate * const dptr;
};

class WriteNotifier : public QSocketNotifier
{
public:
    explicit WriteNotifier(WSerialPortPrivate *d, QObject *parent)
...
static inline void qt_set_databits(termios *tio, WSerialPort::DataBits databits)
{
    tio->c_cflag &= ~CSIZE;
    switch (databits) {
    case WSerialPort::Data5:
        tio->c_cflag |= CS5;
        break;
    case WSerialPort::Data6:
        tio->c_cflag |= CS6;
        break;
    case WSerialPort::Data7:
        tio->c_cflag |= CS7;
        break;
    case WSerialPort::Data8:
        tio->c_cflag |= CS8;
        break;
    default:
        tio->c_cflag |= CS8;
...
static inline void qt_set_parity(termios *tio, WSerialPort::Parity parity)
{
    ...
    case WSerialPort::SpaceParity:
        tio->c_cflag &= ~PARODD;
        tio->c_cflag |= PARENB | CMSPAR;
        break;
    case WSerialPort::MarkParity:
        tio->c_cflag |= PARENB | CMSPAR | PARODD;
        break;
#endif
    case WSerialPort::NoParity:
        tio->c_cflag &= ~PARENB;
        break;
    case WSerialPort::EvenParity:
        tio->c_cflag &= ~PARODD;
        tio->c_cflag |= PARENB;
        break;
    case WSerialPort::OddParity:
...
static inline void qt_set_stopbits(termios *tio, WSerialPort::StopBits stopbits)
{
    switch (stopbits) {
    case WSerialPort::OneStop:
        tio->c_cflag &= ~CSTOPB;
        break;
    case WSerialPort::TwoStop:
        tio->c_cflag |= CSTOPB;
        break;
    default:
        tio->c_cflag &= ~CSTOPB;
...
static inline void qt_set_flowcontrol(termios *tio, WSerialPort::FlowControl flowcontrol)
{
    switch (flowcontrol) {
    case WSerialPort::NoFlowControl:
        tio->c_cflag &= ~CRTSCTS;
        tio->c_iflag &= ~(IXON | IXOFF | IXANY);
        break;
    case WSerialPort::HardwareControl:
        tio->c_cflag |= CRTSCTS;
        tio->c_iflag &= ~(IXON | IXOFF | IXANY);
        break;
    case WSerialPort::SoftwareControl:
        tio->c_cflag &= ~CRTSCTS;
        tio->c_iflag |= IXON | IXOFF | IXANY;
        break;
    default:
...
{
    QString lockFilePath = serialPortLockFilePath(WSerialPortInfoPrivate::portNameFromSystemLocation(systemLocation));
    bool isLockFileEmpty = lockFilePath.isEmpty();
    if (isLockFileEmpty) {
        qWarning("Failed to create a lock file for opening the device");
        setError(WSerialPortErrorInfo(WSerialPort::PermissionError, WSerialPort::tr("Permission error while creating lock file")));
        return false;
    }
    ...
    if (!newLockFileScopedPointer->tryLock()) {
        setError(WSerialPortErrorInfo(WSerialPort::PermissionError, WSerialPort::tr("Permission error while locking the device")));
        return false;
    }
...
WSerialPort::PinoutSignals WSerialPortPrivate::pinoutSignals()
{
    int arg = 0;

    if (::ioctl(descriptor, TIOCMGET, &arg) == -1) {
        setError(getSystemError());
        return WSerialPort::NoSignal;
    }

    WSerialPort::PinoutSignals ret = WSerialPort::NoSignal;

#ifdef TIOCM_LE
    if (arg & TIOCM_LE)
        ret |= WSerialPort::DataSetReadySignal;
#endif
#ifdef TIOCM_DTR
    if (arg & TIOCM_DTR)
        ret |= WSerialPort::DataTerminalReadySignal;
#endif
#ifdef TIOCM_RTS
    if (arg & TIOCM_RTS)
        ret |= WSerialPort::RequestToSendSignal;
#endif
#ifdef TIOCM_ST
    if (arg & TIOCM_ST)
        ret |= WSerialPort::SecondaryTransmittedDataSignal;
#endif
#ifdef TIOCM_SR
    if (arg & TIOCM_SR)
        ret |= WSerialPort::SecondaryReceivedDataSignal;
#endif
#ifdef TIOCM_CTS
    if (arg & TIOCM_CTS)
        ret |= WSerialPort::ClearToSendSignal;
#endif
#ifdef TIOCM_CAR
    if (arg & TIOCM_CAR)
        ret |= WSerialPort::DataCarrierDetectSignal;
...
#ifdef TIOCM_RNG
    if (arg & TIOCM_RNG)
        ret |= WSerialPort::RingIndicatorSignal;
...
bool WSerialPortPrivate::setDataTerminalReady(bool set)
{
...
bool WSerialPortPrivate::setRequestToSend(bool set)
{
...
{
    if (::tcflush(descriptor, (directions == WSerialPort::AllDirections)
                     ? TCIOFLUSH : (directions & WSerialPort::Input) ? TCIFLUSH : TCOFLUSH) == -1) {
        setError(getSystemError());
        return false;
    }
...
bool WSerialPortPrivate::sendBreak(int duration)
{
...
bool WSerialPortPrivate::setBreakEnabled(bool set)
{
...
bool WSerialPortPrivate::setBaudRate()
{
    if (inputBaudRate == outputBaudRate)
        return setBaudRate(inputBaudRate, WSerialPort::AllDirections);

    return (setBaudRate(inputBaudRate, WSerialPort::Input)
        && setBaudRate(outputBaudRate, WSerialPort::Output));
}

bool WSerialPortPrivate::setStandardBaudRate(qint32 baudRate, WSerialPort::Directions directions)
{
#ifdef Q_OS_LINUX
    ...
    if ((directions & WSerialPort::Input) && ::cfsetispeed(&tio, baudRate) < 0) {
        setError(getSystemError());
        return false;
    }

    if ((directions & WSerialPort::Output) && ::cfsetospeed(&tio, baudRate) < 0) {
        setError(getSystemError());
        return false;
    }
...
bool WSerialPortPrivate::setCustomBaudRate(qint32 baudRate, WSerialPort::Directions directions)
{
    if (directions != WSerialPort::AllDirections) {
        setError(WSerialPortErrorInfo(WSerialPort::UnsupportedOperationError,
                                      WSerialPort::tr("Cannot set custom speed for one direction")));
        return false;
    }
    ...
    if (serial.custom_divisor == 0) {
        setError(WSerialPortErrorInfo(WSerialPort::UnsupportedOperationError,
                                      WSerialPort::tr("No suitable custom baud rate divisor")));
        return false;
    }
...
#else
    setError(WSerialPortErrorInfo(WSerialPort::UnsupportedOperationError,
                                  WSerialPort::tr("Custom baud rate is not supported")));
    return false;
#endif
}
...
bool WSerialPortPrivate::setBaudRate(qint32 baudRate, WSerialPort::Directions directions)
{
    if (baudRate <= 0) {
        setError(WSerialPortErrorInfo(WSerialPort::UnsupportedOperationError, WSerialPort::tr("Invalid baud rate value")));
        return false;
    }

    const qint32 unixBaudRate = WSerialPortPrivate::settingFromBaudRate(baudRate);
...
bool WSerialPortPrivate::setDataBits(WSerialPort::DataBits dataBits)
{
    termios tio;
    if (!getTermios(&tio))
        return false;
...
bool WSerialPortPrivate::setParity(WSerialPort::Parity parity)
{
    termios tio;
    if (!getTermios(&tio))
        return false;
...
bool WSerialPortPrivate::setStopBits(WSerialPort::StopBits stopBits)
{
    termios tio;
    if (!getTermios(&tio))
        return false;
...
bool WSerialPortPrivate::setFlowControl(WSerialPort::FlowControl flowControl)
{
    termios tio;
    if (!getTermios(&tio))
        return false;
...
bool WSerialPortPrivate::readNotification()
{
    Q_Q(WSerialPort);
...
bool WSerialPortPrivate::startAsyncWrite()
{
...
bool WSerialPortPrivate::completeAsyncWrite()
{
    Q_Q(WSerialPort);
...
bool WSerialPortPrivate::setTermios(const termios *tio)
{
...
bool WSerialPortPrivate::getTermios(termios *tio)
{
...
bool WSerialPortPrivate::isReadNotificationEnabled() const
{
    return readNotifier && readNotifier->isEnabled();
}

void WSerialPortPrivate::setReadNotificationEnabled(bool enable)
{
    Q_Q(WSerialPort);
...
bool WSerialPortPrivate::isWriteNotificationEnabled() const
{
    return writeNotifier && writeNotifier->isEnabled();
}

void WSerialPortPrivate::setWriteNotificationEnabled(bool enable)
{
    Q_Q(WSerialPort);
...
bool WSerialPortPrivate::waitForReadOrWrite(bool *selectForRead, bool *selectForWrite,
                                           ...
                                           int msecs)
{
    ...
    if (ret == 0) {
        setError(WSerialPortErrorInfo(WSerialPort::TimeoutError));
        return false;
    }
...
qint64 WSerialPortPrivate::readFromPort(char *data, qint64 maxSize)
{
    return qt_safe_read(descriptor, data, maxSize);
}

qint64 WSerialPortPrivate::writeToPort(const char *data, qint64 maxSize)
{
    qint64 bytesWritten = 0;
#if defined(CMSPAR)
    bytesWritten = qt_safe_write(descriptor, data, maxSize);
#else
    if (parity != WSerialPort::MarkParity
            && parity != WSerialPort::SpaceParity) {
        bytesWritten = qt_safe_write(descriptor, data, maxSize);
...
qint64 WSerialPortPrivate::writePerChar(const char *data, qint64 maxSize)
{
    termios tio;
    if (!getTermios(&tio))
        return -1;
    ...
    while (ret < maxSize) {
        ...
        par ^= parity == WSerialPort::MarkParity;
...
qint32 WSerialPortPrivate::settingFromBaudRate(qint32 baudRate)
{
    return standardBaudRateMap().value(baudRate);
}

QList<qint32> WSerialPortPrivate::standardBaudRates()
{
